...
def inscrever_evento(request, id):
    ...
    if request.method == "GET":
        ...
        # TODO: Validar se o usuario é participante
...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        certificados_gerado = Certificado(
            certificado = img_final,
            participante = participante,
            evento = evento
        )

        certificados_gerado.save()
...
def procurar_certificado(request, id):
    ...
    if not certificado:
       messages.add_message(request, constants.ERROR, 'Esse certificados ainda não foi gerado.')
       return redirect(reverse('certificados_evento', kwargs={'id' : evento.id}))
